package com.example.fuelkontrol.service;
...
public class ServicioSerialBluetooth extends Service {
    String respuesta, sultsOK1, sultsOK2, xpermOK1, xpermK1, npermOk1, npermOK2;
    private static final String TAG = ServicioSerialBluetooth.class.getSimpleName();
    ...
    private ConnectedThread mConnectedThread; // Hilo cuando ya se conecto cuando entran y salen datos
    //End Hilos
    ...
    public void onCreate() {
        ...
        if (mAdapter == null) {
            ...
            if (Build.VERSION.SDK_INT > Build.VERSION_CODES.N_MR1) {
                ...
                //Toast.makeText(getApplicationContext(), "Servicio soportado", Toast.LENGTH_SHORT).show();
            }
        }
    }
    ...
    private Notification getNotification(String message) {
        ...
        return new NotificationCompat.Builder(getApplicationContext(), NotificationHelper.CHANNEL_SERVICE_ID)
                ...
                .setSmallIcon(R.drawable.bg_fuel_kontrol)
                .setColor(getResources().getColor(R.color.color_despacho_activo))
    ...
    public class MySerialServiceBinder extends Binder {
        public ServicioSerialBluetooth getService() {
            return ServicioSerialBluetooth.this;
        }
    }
    ...
    private class ConnectThread extends Thread {
        ...
        public ConnectThread(BluetoothDevice device, boolean secure) {
            ...
            try {
                ...
                Toast.makeText(getApplicationContext(), "Socket Type: " + mSocketType + "create() failed" + e, Toast.LENGTH_LONG);
        ...
        //Se le pasa al hilo mas .start
        public void run() {

            //Log.i(TAG, "BEGIN mConnectThread SocketType:" + mSocketType);
            setName("ConnectThread" + mSocketType);

            // Cancelamos siempre la busqueda de dispositivos para evitar que
            // relentize la conexion actual
            ...
            try {
                ...
                try {
                    ...
                    Toast.makeText(getApplicationContext(), "unable to close() " + mSocketType + " socket during connection failure" + e2, Toast.LENGTH_LONG);
        ...
        public void cancel() {
            try {
                ...
                Toast.makeText(getApplicationContext(), "close() of connect " + mSocketType + " socket failed" + e, Toast.LENGTH_LONG);
     ...
     * Responsable de manejar entradas y salidas de conexion
...
//                Log.e(TAG, "temp sockets not created", e);
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    String validando = new String(data, 0, length);
                    Log.i(TAG, "AQUI " + validando);
                    val = validando;
                    enviar(val);
                    Message readMsg = mHandlerActivity.obtainMessage(
                            Constants.MESSAGE_READ, length, -1,
                            data);
                    readMsg.sendToTarget();
                    //int lere = validando.length();
                    //String hide = "01HIDE OK";
                    //Log.i(TAG, lere + "Nume" + hide.length());

                    //String sinSaltosDeLinea = validando.replaceAll("\n", "");
                    //01HIDE OK
                    //Log.i(TAG, hide);
                    //Log.i(TAG, "." + sinSaltosDeLinea + "." + "." + hide + ".");
                    //sinSaltosDeLinea.matches("01HIDE OK.*");
                    //sHIDE OK\n\r
                   /* if (validando.equals("01HIDE OK\n\r")) {
                        Log.e(TAG, "no correcto" + valor);
                        serialWriteString("01XPERM\n");

                    } else {
                        Log.e(TAG, "Se valido correcto" + valor);

                    }
                   /* if (sinSaltosDeLinea.matches("01HIDE OK.*")) {
                        Log.e(TAG, "no correcto" + valor);
                    } else {
                        Log.e(TAG, "Se valido correcto" + valor);
    ...
    private void serialLeerBytes() {
        ConnectedThread readd;
        synchronized (this) {
            if (mState != STATE_CONNECTED) return;
            readd = mConnectedThread;
        }
        readd.leerDato();
    }
    ...
    public void serialWriteString(String s) {
        ...
        //String sinSaltosDeLinea = mConnectedThread.val.replaceAll("\n", "");
    ...
    public String enviar(String valor) {
        return mConnectedThread.val;
    }
    ...
    public void iniciarDespacho(Context actividad, int manguera, String comando,
                                double litros) {
        //Concatenar el comando con manguera+comando+litros
        String cadenaHide = manguera + comando;
        String cadenaSults = manguera + comando + " " + litros;
        String cadenaXperm = manguera + comando;
        String cadenaNperm = manguera + comando;
        String cadenaStop = manguera + comando;
        String cadenaGlits = manguera + comando;
        String cadenaShow = manguera + comando;

        String compararHide = manguera + "HIDE OK";
        String compararSults = manguera + comando + " " + litros;
        String compararXperm = manguera + "XPERM OK";
        String compararShow = manguera + "SHOW OK";
        String compararNperm = manguera + "NPERM OK";
        String compararStop = manguera + "STOP OK";
        String compararGlits = manguera + "GLITS OK";
        String vacio = "";
        byte bufferVacio[] = vacio.getBytes();
        byte bufferHide[] = cadenaHide.getBytes();
        byte bufferSults[] = cadenaSults.getBytes();
        byte bufferXperm[] = cadenaXperm.getBytes();
        byte bufferNperm[] = cadenaNperm.getBytes();
        byte bufferStop[] = cadenaStop.getBytes();
        byte bufferGlits[] = cadenaGlits.getBytes();
        int iIntentos = 0;
